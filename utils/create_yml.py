        ...
        for db_field in db_fields:
            if isinstance(doc[db_field], (list, tuple)):
                ...
                if db_field == "go_config":
                    doc[db_field]["path"] = os.path.join(
                        tools_path, doc[db_field]["path"]
                    )
                else:
...
if __name__ == "__main__":
    ...
    with open(config_file, "r") as config_yml:
        config = YAML(typ="safe").load(config_yml)
        ...
        if args.fr.endswith(".fastq.gz") and args.rr.endswith(".fastq.gz"):
            ...
            config["both_reads"] = [
                args.fr.split("/")[-1].split(".fastq.gz")[0],
                args.rr.split("/")[-1].split(".fastq.gz")[0],
            ]

        else:
            forward_reads = (
                args.ena_raw_data_path + "/" + args.run_accession_number + "_1.fastq.gz"
            )
            reverse_reads = (
                args.ena_raw_data_path + "/" + args.run_accession_number + "_2.fastq.gz"
            )
            ...
            config["both_reads"] = [
                forward_reads.split("/")[-1].split(".fastq.gz")[0],
                reverse_reads.split("/")[-1].split(".fastq.gz")[0],
            ]
